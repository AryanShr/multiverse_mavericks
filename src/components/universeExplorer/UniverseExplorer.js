  ...
  return (
    ...
    <Link to={`/${route}`}>
        <a onMouseEnter={()=>setShow(true)} onMouseLeave = {()=>setShow(false)}>
            <img src= {content} style={show?{display:'block'}:{display:'none'}} className='absolute z-10 h-[8rem] w-[15rem]  transition-all pointer-events-none top-16 left-16' alt='galaxy'/>
